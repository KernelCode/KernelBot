...
#  sets up a systemd service, and starts the agent.
#
...
echo -e "${CYAN}║    AI Agent System · Bun · Chromium       ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════╝${NC}"
...
if [[ "$SKIP_ENV" != "true" ]]; then
  ...
  echo "    TELEGRAM_BOT_TOKEN    — from @BotFather (for Telegram integration)"
  echo "    OWNER_TELEGRAM_ID     — your numeric user ID (optional)"
  echo ""
  ...
  while true; do
    ...
    if [[ -z "$BOT_TOKEN" ]]; then
      ...
      warn "Token doesn't look valid (expected format: 123456:ABC-DEF...)"
  ...
  if [[ -n "$OWNER_ID" && ! "$OWNER_ID" =~ ^[0-9]+$ ]]; then
    warn "Owner ID should be a number."
    read -rp "  Use it anyway? [y/N] " yn
    [[ "$yn" =~ ^[Yy]$ ]] || OWNER_ID=""
  fi
  ...
  if [[ -z "$ANTHROPIC_KEY" && -z "$OPENAI_KEY" && -z "$GOOGLE_KEY" && -z "$GROQ_KEY" ]]; then
    warn "No AI provider key set. KERNEL won't work without at least one."
    read -rp "  Continue anyway? [y/N] " yn
  ...
  # Write .env
  cat > "$ENV_FILE" <<EOF
# KernelBot — Environment Variables
...
[Unit]
Description=KernelBot AI Agent
...
if [[ "$EXISTING_SERVICE" == "true" ]] && systemctl is-active --quiet "$SERVICE_NAME"; then
  read -rp "  KernelBot is currently running. Restart with new changes? [Y/n] " yn
  if [[ "$yn" =~ ^[Nn]$ ]]; then
...
if systemctl is-active --quiet "$SERVICE_NAME" 2>/dev/null || true; then
  ...
  if systemctl is-active --quiet "$SERVICE_NAME"; then
    ...
    echo ""
    warn "Common fixes:"
    echo "    1. Check your config:     cat $ENV_FILE"
